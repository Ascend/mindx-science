...
namespace {
const char *kRFFT3_SP = "RFFT3_SP";
const uint32_t kOutputNum = 8;
const uint32_t kInputNum = 1;
}  // namespace

namespace aicpu {
uint32_t RFFT3SPCpuKernel::Compute(CpuKernelContext &ctx) {
  KERNEL_HANDLE_ERROR(NormalCheck(ctx, kInputNum, kOutputNum),
                      "[%s] check input and output failed.", kRFFT3_SP);
  ...
  switch (data_type) {
    case DT_FLOAT:
      return RFFT3SPCompute<float, complex<float>>(ctx);
    case DT_DOUBLE:
      return RFFT3SPCompute<double, complex<double>>(ctx);
    default:
      KERNEL_LOG_ERROR("RFFT3_SP kernel data type [%s] not support.",
...
uint32_t RFFT3SPComputeSingleBatch(CpuKernelContext &ctx, int64_t batch_idx) {
  ...
  DSizes<DenseIndex, FFTDim> zero_start{ 0, 0, 0 };
  ...
  DSizes<DenseIndex, FFTDim> modes_sizes{ modes[0], modes[1], modes[N2] };
...
template <typename T, typename C>
uint32_t RFFT3SPCpuKernel::RFFT3SPCompute(CpuKernelContext &ctx) {
  const int64_t FFTDim = 3;
  vector<int64_t> shape_x = ctx.Input(0)->GetTensorShape()->GetDimSizes();
  int64_t dim0 = 1;
  ...
  auto shard_rfft3_sp = [&](int64_t start, int64_t end) {
    for (int64_t batch_idx = start; batch_idx < end; batch_idx++) {
      RFFT3SPComputeSingleBatch<T, C>(ctx, batch_idx);
    }
  };
  KERNEL_HANDLE_ERROR(CpuKernelUtils::ParallelFor(ctx, dim0, dim0 / max_core, shard_rfft3_sp),
                      "RFFT3_SP Compute failed.")
  return KERNEL_STATUS_OK;
}

REGISTER_CPU_KERNEL(kRFFT3_SP, RFFT3SPCpuKernel);
}  // namespace aicpu
