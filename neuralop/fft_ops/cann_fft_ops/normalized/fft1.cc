...
namespace {
const char *kFFT1 = "FFT1";
const uint32_t kOutputNum = 2;
const uint32_t kInputNum = 2;
}  // namespace

namespace aicpu {
uint32_t FFT1CpuKernel::Compute(CpuKernelContext &ctx) {
  KERNEL_HANDLE_ERROR(NormalCheck(ctx, kInputNum, kOutputNum),
                      "[%s] check input and output failed.", kFFT1);
  ...
  switch (data_type) {
    case DT_FLOAT:
      return FFT1Compute<float>(ctx);
    case DT_DOUBLE:
      return FFT1Compute<double>(ctx);
    default:
      KERNEL_LOG_ERROR("FFT1 kernel data type [%s] not support.",
...
template <typename T>
uint32_t FFT1ComputeSingleBatch(CpuKernelContext &ctx, int64_t batch_idx) {
...
template <typename T>
uint32_t FFT1CpuKernel::FFT1Compute(CpuKernelContext &ctx) {
  ...
  auto shard_fft1 = [&](int64_t start, int64_t end) {
    for (int64_t batch_idx = start; batch_idx < end; batch_idx++) {
      FFT1ComputeSingleBatch<T>(ctx, batch_idx);
    }
  };
  KERNEL_HANDLE_ERROR(CpuKernelUtils::ParallelFor(ctx, dim0, dim0 / max_core, shard_fft1),
                      "FFT1 Compute failed.")
  return KERNEL_STATUS_OK;
}

REGISTER_CPU_KERNEL(kFFT1, FFT1CpuKernel);
}  // namespace aicpu
