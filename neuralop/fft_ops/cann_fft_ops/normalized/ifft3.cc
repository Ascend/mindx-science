...
namespace {
const char *kIFFT3 = "IFFT3";
const uint32_t kOutputNum = 2;
const uint32_t kInputNum = 2;
}  // namespace

namespace aicpu {
uint32_t IFFT3CpuKernel::Compute(CpuKernelContext &ctx) {
  const int32_t FFTDim = 3;
  KERNEL_HANDLE_ERROR(NormalCheck(ctx, kInputNum, kOutputNum),
                      "[%s] check input and output failed.", kIFFT3);
  ...
  switch (data_type) {
    case DT_FLOAT:
      return IFFT3Compute<float>(ctx);
    case DT_DOUBLE:
      return IFFT3Compute<double>(ctx);
    default:
      KERNEL_LOG_ERROR("IFFT3 kernel data type [%s] not support.",
...
template <typename T>
uint32_t IFFT3ComputeSingleBatch(CpuKernelContext &ctx, int64_t batch_idx) {
  const int32_t FFTDim = 3;
...
template <typename T>
uint32_t IFFT3CpuKernel::IFFT3Compute(CpuKernelContext &ctx) {
  ...
  auto shard_ifft3 = [&](int64_t start, int64_t end) {
    for (int64_t batch_idx = start; batch_idx < end; batch_idx++) {
      IFFT3ComputeSingleBatch<T>(ctx, batch_idx);
    }
  };
  KERNEL_HANDLE_ERROR(CpuKernelUtils::ParallelFor(ctx, dim0, dim0 / max_core, shard_ifft3),
                      "IFFT3 Compute failed.")
  return KERNEL_STATUS_OK;
}

REGISTER_CPU_KERNEL(kIFFT3, IFFT3CpuKernel);
}  // namespace aicpu
