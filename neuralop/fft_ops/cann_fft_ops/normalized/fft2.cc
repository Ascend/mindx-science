...
namespace aicpu {
uint32_t FFT2CpuKernel::Compute(CpuKernelContext &ctx) {
  const int32_t FFTDim = 2;
  KERNEL_HANDLE_ERROR(NormalCheck(ctx, kInputNum, kOutputNum),
                      "[%s] check input and output failed.", kFFT2);
  ...
  switch (data_type) {
    case DT_FLOAT:
      return FFT2Compute<float>(ctx);
    case DT_DOUBLE:
      return FFT2Compute<double>(ctx);
    default:
      KERNEL_LOG_ERROR("FFT2 kernel data type [%s] not support.",
...
template <typename T>
uint32_t FFT2ComputeSingleBatch(CpuKernelContext &ctx, int64_t batch_idx) {
  const int32_t FFTDim = 2;
...
template <typename T>
uint32_t FFT2CpuKernel::FFT2Compute(CpuKernelContext &ctx) {
  ...
  auto shard_fft2 = [&](int64_t start, int64_t end) {
    for (int64_t batch_idx = start; batch_idx < end; batch_idx++) {
      FFT2ComputeSingleBatch<T>(ctx, batch_idx);
    }
  };
  KERNEL_HANDLE_ERROR(CpuKernelUtils::ParallelFor(ctx, dim0, dim0 / max_core, shard_fft2),
                      "FFT2 Compute failed.")
  return KERNEL_STATUS_OK;
}

REGISTER_CPU_KERNEL(kFFT2, FFT2CpuKernel);
}  // namespace aicpu
