...
namespace {
const char *kRFFT2_SP = "RFFT2_SP";
const uint32_t kOutputNum = 4;
const uint32_t kInputNum = 1;
}  // namespace

namespace aicpu {
uint32_t RFFT2SPCpuKernel::Compute(CpuKernelContext &ctx) {
  KERNEL_HANDLE_ERROR(NormalCheck(ctx, kInputNum, kOutputNum),
                      "[%s] check input and output failed.", kRFFT2_SP);
  ...
  switch (data_type) {
    case DT_FLOAT:
      return RFFT2SPCompute<float, complex<float>>(ctx);
    case DT_DOUBLE:
      return RFFT2SPCompute<double, complex<double>>(ctx);
    default:
      KERNEL_LOG_ERROR("RFFT2_SP kernel data type [%s] not support.",
...
uint32_t RFFT2SPComputeSingleBatch(CpuKernelContext &ctx, int64_t batch_idx) {
  ...
  DSizes<DenseIndex, FFTDim> zero_start{0, 0};
...
template <typename T, typename C>
uint32_t RFFT2SPCpuKernel::RFFT2SPCompute(CpuKernelContext &ctx) {
  const int64_t FFTDim = 2;
  vector<int64_t> shape_x = ctx.Input(0)->GetTensorShape()->GetDimSizes();
  int64_t dim0 = 1;
  ...
  auto shard_rfft2_sp = [&](int64_t start, int64_t end) {
    for (int64_t batch_idx = start; batch_idx < end; batch_idx++) {
      RFFT2SPComputeSingleBatch<T, C>(ctx, batch_idx);
    }
  };
  KERNEL_HANDLE_ERROR(CpuKernelUtils::ParallelFor(ctx, dim0, dim0 / max_core, shard_rfft2_sp),
                      "RFFT2_SP Compute failed.")
  return KERNEL_STATUS_OK;
}

REGISTER_CPU_KERNEL(kRFFT2_SP, RFFT2SPCpuKernel);
}  // namespace aicpu
