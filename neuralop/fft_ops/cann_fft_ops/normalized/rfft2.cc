...
namespace aicpu {
uint32_t RFFT2CpuKernel::Compute(CpuKernelContext &ctx) {
  KERNEL_HANDLE_ERROR(NormalCheck(ctx, kInputNum, kOutputNum),
                      "[%s] check input and output failed.", kRFFT2);
  ...
  switch (data_type) {
    case DT_FLOAT:
      return RFFT2Compute<float, complex<float>>(ctx);
    case DT_DOUBLE:
      return RFFT2Compute<double, complex<double>>(ctx);
    default:
      KERNEL_LOG_ERROR("RFFT2 kernel data type [%s] not support.",
...
uint32_t RFFT2ComputeSingleBatch(CpuKernelContext &ctx, int64_t batch_idx) {
  ...
  TensorMap<Eigen::Tensor<T, N2, RowMajor>> cur_x_tensor(cur_x_ptr, dim1_x, dim2_x);
  Eigen::Tensor<C, N2, RowMajor> cur_y_tensor(dim1_y, dim2_y);
  DSizes<DenseIndex, FFTDim> in_sizes{s[0], s[1]};
  DSizes<DenseIndex, FFTDim> output_slice_sizes{dim1_y, dim2_y};
  ...
  TensorMap<Eigen::Tensor<T, N2, RowMajor>> cur_y_re_tensor(cur_y_re_ptr, dim1_y, dim2_y);
  TensorMap<Eigen::Tensor<T, N2, RowMajor>> cur_y_im_tensor(cur_y_im_ptr, dim1_y, dim2_y);
...
template <typename T, typename C>
uint32_t RFFT2CpuKernel::RFFT2Compute(CpuKernelContext &ctx) {
  const int64_t FFTDim = 2;
  vector<int64_t> shape_x = ctx.Input(0)->GetTensorShape()->GetDimSizes();
  int64_t dim0 = 1;
  ...
  auto shard_rfft2 = [&](int64_t start, int64_t end) {
    for (int64_t batch_idx = start; batch_idx < end; batch_idx++) {
      RFFT2ComputeSingleBatch<T, C>(ctx, batch_idx);
    }
  };
  KERNEL_HANDLE_ERROR(CpuKernelUtils::ParallelFor(ctx, dim0, dim0 / max_core, shard_rfft2),
                      "RFFT2 Compute failed.")
  return KERNEL_STATUS_OK;
}

REGISTER_CPU_KERNEL(kRFFT2, RFFT2CpuKernel);
}  // namespace aicpu
