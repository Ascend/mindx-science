...
namespace {
const char *kIRFFT3 = "IRFFT3";
const uint32_t kOutputNum = 1;
const uint32_t kInputNum = 2;
}  // namespace

namespace aicpu {
uint32_t IRFFT3CpuKernel::Compute(CpuKernelContext &ctx) {
  KERNEL_HANDLE_ERROR(NormalCheck(ctx, kInputNum, kOutputNum),
                      "[%s] check input and output failed.", kIRFFT3);
  ...
  switch (data_type) {
    case DT_FLOAT:
      return IRFFT3Compute<float, complex<float>>(ctx);
    case DT_DOUBLE:
      return IRFFT3Compute<double, complex<double>>(ctx);
    default:
      KERNEL_LOG_ERROR("IRFFT3 kernel data type [%s] not support.",
...
uint32_t IRFFT3ComputeSingleBatch(CpuKernelContext &ctx, int64_t batch_idx) {
  ...
  TensorMap<Eigen::Tensor<T, N3, RowMajor>> cur_x_re_tensor(cur_x_re_ptr, dim1_x, dim2_x, dim3_x);
  TensorMap<Eigen::Tensor<T, N3, RowMajor>> cur_x_im_tensor(cur_x_im_ptr, dim1_x, dim2_x, dim3_x);
  Eigen::Tensor<C, N3, RowMajor> cur_x_tensor(dim1_x, dim2_x, dim3_x);
  ...
  TensorMap<Eigen::Tensor<T, N3, RowMajor>> cur_y_tensor(cur_y_ptr, dim1_y, dim2_y, dim3_y);
...
template <typename T, typename C>
uint32_t IRFFT3CpuKernel::IRFFT3Compute(CpuKernelContext &ctx) {
  const int64_t FFTDim = 3;
  vector<int64_t> shape_x = ctx.Input(0)->GetTensorShape()->GetDimSizes();
  int64_t dim0 = 1;
  ...
  auto shard_irfft3 = [&](int64_t start, int64_t end) {
    for (int64_t batch_idx = start; batch_idx < end; batch_idx++) {
      IRFFT3ComputeSingleBatch<T, C>(ctx, batch_idx);
    }
  };
  KERNEL_HANDLE_ERROR(CpuKernelUtils::ParallelFor(ctx, dim0, dim0 / max_core, shard_irfft3),
                      "IRFFT3 Compute failed.")
  return KERNEL_STATUS_OK;
}

REGISTER_CPU_KERNEL(kIRFFT3, IRFFT3CpuKernel);
}  // namespace aicpu
